/*!
 * ngImgCrop v0.3.3
 * https://github.com/hmamede/ngImgCrop
 *
 * Copyright (c) 2014 Helder Mamede
 * License: MIT
 *
 * Generated at Wednesday, January 28th, 2015, 11:30:12 AM
 */
(function () {
    'use strict';
    ...
    crop.factory('cropAreaCircle', ['cropArea', function (CropArea) {
        ...
        CropAreaCircle.prototype.draw = function () {
            CropArea.prototype.draw.apply(this, arguments, this._name);
    ...
    crop.factory('cropAreaRectangle', ['cropArea', function (CropArea) {
        ...
        CropAreaRectangle.prototype._calcRectangleDimensions = function () {
            var hSizeX = this._size / 2;
            var hSizeY = (this._size * 0.75 / 2);
            return {
                left: this._x - hSizeX,
                top: this._y - hSizeY,
                right: this._x + hSizeX,
        ...
        CropAreaRectangle.prototype._drawArea = function (ctx, centerCoords, size) {
            var hSizeX = size / 2;
            var hSizeY = (size / 2) * 0.75;
            ctx.rect(centerCoords[0] - hSizeX, centerCoords[1] - hSizeY, hSizeX * 2, hSizeY * 2);
        };
        ...
        CropAreaRectangle.prototype._dontDragOutside = function () {
            ...
            if (this._y < this._size * 0.75 / 2) { this._y = this._size * 0.75 / 2; }
            if (this._y > h - this._size * 0.75 / 2) { this._y = h - this._size * 0.75 / 2; }
        };
    ...
    crop.factory('cropArea', ['cropCanvas', function (CropCanvas) {
        ...
        CropArea.prototype.draw = function () {
            // draw crop area
            this._cropCanvas.drawCropArea(this._image, [this._x, this._y], this._size, this._drawArea, this._name);
        };
    ...
    crop.factory('cropCanvas', [function () {
        ...
        return function (ctx) {
            ...
            this.drawCropArea = function (image, centerCoords, size, fnDrawClipPath, name) {
                var xRatio = image.width / ctx.canvas.width,
                    yRatio = image.height / ctx.canvas.height,
                    xLeft = centerCoords[0] - size / 2,
                    yTop = name == "rectangle" ? centerCoords[1] - size * 0.75 / 2 : centerCoords[1] - size / 2;
                ...
                if (size > 0) {
                    if (name == "rectangle") {
                        ctx.drawImage(image, xLeft * xRatio, yTop * yRatio, size * xRatio, size * 0.75 * yRatio, xLeft, yTop, size, size * 0.75);
                    }
                    else {
    ...
    crop.factory('cropHost', ['$document', 'cropAreaCircle', 'cropAreaSquare', 'cropAreaRectangle', 'cropEXIF', function ($document, CropAreaCircle, CropAreaSquare, CropAreaRectangle, cropEXIF) {
        ...
        return function (elCanvas, opts, events) {
            ...
            this.getResultImageDataURI = function () {
                ...
                var percRectangle = theArea._name == 'rectangle' ? 0.75 : 1;
                temp_canvas = angular.element('<canvas></canvas>')[0];
                temp_ctx = temp_canvas.getContext('2d');
                temp_canvas.width = resImgSize;
                temp_canvas.height = resImgSize * percRectangle;
                if (image !== null) {
                    temp_ctx.drawImage(image,
                                       (theArea.getX() - theArea.getSize() / 2) * (image.width / ctx.canvas.width),
                                       (theArea.getY() - theArea.getSize() * percRectangle / 2) * (image.height / ctx.canvas.height),
                                       theArea.getSize() * (image.width / ctx.canvas.width),
                                       theArea.getSize() * percRectangle * (image.height / ctx.canvas.height),
                                       0,
                                       0,
                                       temp_canvas.width,
                                       temp_canvas.height);
